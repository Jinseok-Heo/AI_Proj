...
def read_dataset(path, max_length=10):
    ...
    """
    o, x, b = [], [], []
    tf.compat.v1.logging.info("Read dataset: " + path)
...
class BinPackingDataLoader(object):
    ...
    def _create_input_queue(self):
        ...
        name = 'train'
        self.order_ops[name] = tf.compat.v1.placeholder(tf.int32, shape=[])
        self.input_ops[name] = tf.compat.v1.placeholder(tf.float32, shape=[None, None])
        self.target_ops[name] = tf.compat.v1.placeholder(tf.int32, shape=[])

        min_after_dequeue = 1000
        capacity = min_after_dequeue + 3 * self.batch_size

        # Create a RandomShuffleQueue that pulls data from queue randomly
        self.queue_ops[name] = tf.queue.RandomShuffleQueue(
            capacity=capacity,
        ...
        self.queue_ops[name] = tf.queue.FIFOQueue(
            capacity=capacity,
            dtypes=[tf.int32, tf.float32, tf.int32],
            shapes=[[], [self.max_length, 3, ], []],
    ...
    def run_input_queue(self, sess):
        ...
        for name in self.data_num.keys():
            ...
            tf.compat.v1.logging.info("Thread for [{}] start".format(name))
    ...
    def stop_input_queue(self):
        ...
        tf.compat.v1.logging.info("All threads stopped")
    ...
    def read_zip_and_update_data_new(self, path, name):
        ...
        :return:
        """
        tf.compat.v1.logging.info("Read dataset")
